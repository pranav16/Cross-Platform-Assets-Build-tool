...
NewAssetTypeInfo( "effects",
  {
    ...
    RegisterReferencedAssets = function( i_sourceRelativePath )
      ...
      if DoesFileExist( sourceAbsolutePath ) then
		...
		local path_vertexShader = shaders['vertexShader']
		local path_fragmentShader = shaders['fragmentShader']
...
NewAssetTypeInfo( "materials",
  {
    ...
    RegisterReferencedAssets = function( i_sourceRelativePath )
      ...
      if DoesFileExist( sourceAbsolutePath ) then
		...
		local texture = material['Parameters']['Texture']
        RegisterAssetToBeBuilt( effect, "effects")
		if(texture ~= nil) then
