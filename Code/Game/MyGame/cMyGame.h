...
namespace eae6320
{
	...
	private:
		eae6320::Graphics::Camera* m_currentCamera;
		int m_currentRow;
		std::vector<float>currentAngles;
		// Configuration
		//--------------
		std::vector<eae6320::Graphics::RenderData*> listOfRenderData;
		std::vector<eae6320::Graphics::SpriteRenderData*> listOfSpriteRenderData;
		// You can configure your game by overriding the following functions
		
		virtual const char* GetPathToLogTo() const { return "Game.log"; }
#if defined( EAE6320_PLATFORM_WINDOWS )
		...
		virtual const char* GetMainWindowClassName() const { return "Pranav's EAE6320 Main Window Class"; }
		...
		void moveCurrentLeftRow();
		void moveCurrentRightRow();
		// Initialization / Clean Up
		//--------------------------
		float currentAngle;
