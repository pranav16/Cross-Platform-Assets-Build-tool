		...
		if ( fout.is_open() )
		{
			 ...
			 "verticies = { \n";
			{
				...
				for (size_t i = 0; i < i_vertexBuffer.size(); i++)
				{
					...
					float x = i_vertexBuffer[i].vertex.x;
					float y = i_vertexBuffer[i].vertex.y;
					float z = i_vertexBuffer[i].vertex.z;
					float r = i_vertexBuffer[i].vertex.r;
					float g = i_vertexBuffer[i].vertex.g;
					float b = i_vertexBuffer[i].vertex.b;
					float a = i_vertexBuffer[i].vertex.a;
					float u = i_vertexBuffer[i].vertex.u;
					float v = i_vertexBuffer[i].vertex.v;
					sprintf_s(buffer, sizeof(buffer), "%f,%f,%f", x, y, z);
					fout << buffer;
					fout << "}, \n";
					fout << "uvs={";
					sprintf_s(buffer, sizeof(buffer), "%f,%f",u,v);
					fout << buffer;
					fout << "}, \n";
					fout << "color = {";
					sprintf_s(buffer, sizeof(buffer), "%f,%f,%f,%f", r, g, b, a);
					fout << buffer;
					fout << "}\n }, \n";
				...
				for (size_t i = 0; i < i_indexBuffer.size(); i++)
				{
					if(indexCount == 0)
					fout << "{ \n";
					unsigned int index = static_cast<unsigned int>(i_indexBuffer[i]);
					char buffer[100];
