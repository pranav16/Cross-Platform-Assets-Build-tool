...
void eae6320::Graphics::cRenderState::Bind() const
{
	// Alpha Transparency
	{
		...
		g_direct3dImmediateContext->OMSetBlendState( m_blendState, noBlendFactor, defaultSampleMask );
	}
	// Depth Buffering
	{
		EAE6320_ASSERT( m_depthStencilState != NULL );
		const unsigned int unusedStencilReference = 0;
		g_direct3dImmediateContext->OMSetDepthStencilState( m_depthStencilState, unusedStencilReference );
	}
	// Draw Both Triangle Sides
	{
		EAE6320_ASSERT( m_rasterizerState != NULL );
		g_direct3dImmediateContext->RSSetState( m_rasterizerState );
	}
}
...
bool eae6320::Graphics::cRenderState::InitializeFromBits()
{
	...
	// Alpha Transparency
	{
		...
		const HRESULT result = g_direct3dDevice->CreateBlendState( &blendStateDescription, &m_blendState );
		if ( FAILED( result ) )
		{
	...
	// Depth Buffering
	{
		...
		const HRESULT result = g_direct3dDevice->CreateDepthStencilState( &depthStateDescription, &m_depthStencilState );
		if ( FAILED( result ) )
		{
			wereThereErrors = true;
	...
	// Draw Both Triangle Sides
	{
		...
		const HRESULT result = g_direct3dDevice->CreateRasterizerState( &rasterizerStateDescription, &m_rasterizerState );
		if ( FAILED( result ) )
		{
			wereThereErrors = true;
