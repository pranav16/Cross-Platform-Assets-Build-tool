...
void eae6320::Graphics::cSprite::Draw() const
{
	...
	// Update the vertex buffer
	{
		...
		void* memoryToWriteTo;
		{
			D3D11_MAPPED_SUBRESOURCE mappedSubResource;
			{
				...
				const HRESULT result = g_direct3dImmediateContext->Map( ms_vertexBuffer, noSubResources, mapType, noFlags, &mappedSubResource );
				if ( FAILED( result ) )
				{
		...
		{
			const unsigned int noSubResources = 0;
			g_direct3dImmediateContext->Unmap( ms_vertexBuffer, noSubResources );
		}
	}
	...
	// Bind the vertex buffer to the device as a data source
	{
		...
		g_direct3dImmediateContext->IASetVertexBuffers( startingSlot, vertexBufferCount, &ms_vertexBuffer, &bufferStride, &bufferOffset );
	}
	...
	// Specify what kind of data the vertex buffer holds
	{
		...
		g_direct3dImmediateContext->IASetPrimitiveTopology( D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP );
	}
	...
	// Render triangles from the currently-bound vertex buffer
	{
		...
		g_direct3dImmediateContext->Draw( s_vertexCount, indexOfFirstVertexToUse );
	}
}
...
bool eae6320::Graphics::cSprite::Initialize()
{
	...
	const HRESULT result = g_direct3dDevice->CreateBuffer( &bufferDescription, &initialData, &ms_vertexBuffer );
	if ( SUCCEEDED( result ) )
	{
		return true;
	}
