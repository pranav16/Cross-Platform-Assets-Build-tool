...
void eae6320::Graphics::cTexture::Bind( const unsigned int i_id ) const
{
	...
	g_direct3dImmediateContext->PSSetShaderResources( i_id, viewCount, &m_textureView );
}
...
bool eae6320::Graphics::cTexture::Initialize( const char* const i_path, const sDdsInfo& i_ddsInfo )
{
	...
	}
	{
		...
		const HRESULT result = g_direct3dDevice->CreateTexture2D( &textureDescription, subResourceData, &resource );
		if ( FAILED( result ) )
		{
			wereThereErrors = true;
	...
	// Create the view
	{
		...
		const HRESULT result = g_direct3dDevice->CreateShaderResourceView( resource, &shaderResourceViewDescription, &m_textureView );
		if ( FAILED( result ) )
		{
			wereThereErrors = true;
